...
> [!TIP]
> To deploy this project using **GUI-based flow**, navigate to [console](https://console.stacktape.com/create-new-project/git-project-using-console?name=my-stacktape-app&repositoryType=public&repositoryUrl=https://github.com/stacktape/starter-gpu-inference-api)

- GPU-accelerated batch inference using [batch jobs](https://docs.stacktape.com/compute-resources/batch-jobs/) with
  NVIDIA GPUs.
- Upload data to S3, trigger an inference job, and get results. Uses spot instances for up to 90% cost savings.
- Perfect for ML inference, image processing, video transcoding, or any GPU-intensive workload.
...
```bash
stacktape init --starterId gpu-inference-api
```
...
[dev command](https://docs.stacktape.com/cli/commands/dev/). For example, to develop and debug lambda function `triggerInference`, you can use

```bash
stacktape dev --region <<your-region>> --stage <<stage>> --resourceName triggerInference
```
